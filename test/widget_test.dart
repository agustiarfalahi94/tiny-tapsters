...
void main() {
  ...
  testWidgets('bubble pop header never shows a negative remaining count', (
    ...
    expect(
      ...
      reason: 'the extra tap must not have produced another pop',
    );

    // Flush the pending win timer so the test does not end with a dangling
    // timer outstanding.
  ...
  testWidgets('count game: wrong tap never advances, correct tap does', (
    ...
    // The correct tap highlights, then advances after the ~350ms delay.
  ...
  testWidgets('count game: double tap during the advance delay counts once', (
    ...
    ); // inside the 350ms advance window
