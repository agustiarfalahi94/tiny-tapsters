...
void main() {
  ...
  group('per-screen music level', () {
    test('attenuation lowers the music without silencing it', () async {
      final sink = FakeMusicSink();
      final music = MusicService.withSink(sink);

      await music.play(MusicTrack.game);
      final normal = sink.volume!;

      await music.setAttenuation(0.35);
      expect(sink.volume, lessThan(normal));
      // Turned down, not off. A game that falls silent reads as broken.
      expect(sink.volume, greaterThan(0));

      await music.setAttenuation(1);
      expect(sink.volume, normal);
    });

    test('a quiet screen and a ducking sound stack', () async {
      final sink = FakeMusicSink();
      final music = MusicService.withSink(sink);

      await music.play(MusicTrack.game);
      await music.setAttenuation(0.35);
      final quiet = sink.volume!;

      await music.duck();
      expect(sink.volume, lessThan(quiet));

      await music.unduck();
      expect(sink.volume, quiet);
    });
  });
  ...
  test('the wrong-answer sound is generated and bundled', () {
    // Generated by tool/generate_sfx.dart, not downloaded, so it is easy for
    // a regenerate to write it somewhere pubspec does not list.
    expect(File('assets/sfx/wrong.wav').existsSync(), isTrue);
    expect(
      File('pubspec.yaml').readAsStringSync(),
      contains('assets/sfx/wrong.wav'),
    );
  });
