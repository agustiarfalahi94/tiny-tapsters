...
class SpeechListenOptions {
  ...
  /// Android's *short* "probably finished" silence timer. Defaults to roughly
  /// half a second on most devices, which is shorter than the pause between
  /// words, so a session ends mid-sentence. Unpatched, this is not settable at
  /// all. Android only.
  final Duration? possiblyCompleteSilence;

  /// TINY TAPSTERS PATCH — the minimum length of a recognition session, so a
  ...
  SpeechListenOptions({
    ...
    /// TINY TAPSTERS PATCH — Android's short "probably finished" silence
    /// timer. See the field docs above.
    this.possiblyCompleteSilence = null,
